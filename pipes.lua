...
-- lightest/darkest paint colours: only allow items in if they match what's
-- already in slot 1 of the container (single-item chest)
...
-- 3rd-row paint colours: only allow items in if they belong to the category
...
local function evaluateChestPriority( container, itemUid )
	...
	-- container is single-item type and item matches what's already inside
	if isAnyOf( color, oneItemContainer ) then
		local firstItem = sm.container.getFirstItem( container.shape:getInteractable():getContainer() )
		-- an empty single-item chest has nothing "already inside" to match yet;
		-- treat it as a valid destination (it'll define its own contents once it
		-- receives its first item) rather than indexing a nil return and crashing
		if firstItem == nil or firstItem["uuid"] == itemUid then
			return 3
		end
	end
